...
It can be seen from Figure \ref{fig:data_acquisition_strategies} that we use two strategies to build our dataset. Our primary data were taken using ..., while our secondary data were collected from various websites.
...
A total of 1,247 high-resolution images ranging from 1920x1080 to 4096x3072 pixels of Medaka fish (Oryzias species) were obtained, and were annotated using ..., resulting in a total of 723 instances of \textit{Oryzias celebensis} and 524 instances of \textit{Oryzias javanicus}. Figure \ref{fig:dataset_hybrid} shows the examples of annotated Medaka images from our dataset, both from the primary and the secondary data.
...
As illustrated in Fugure \ref{fig:data_acquisition_strategies}, after both of raw images and raw photos were collected, we then filtered the images based on the file size and the image PSNR.
