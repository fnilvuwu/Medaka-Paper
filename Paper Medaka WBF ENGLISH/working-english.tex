...
After applying 5-fold cross-validation, the predictions from the trained five YOLOv8 models were combined using ensemble techniques. The three ensemble methods used were Weighted Box Fusion (WBF), Non-Maximum Suppression, dan Non-Maximum Weighted (NMW).

\subsubsection{Non-Maximum Suppression (NMS)}
Non-Maximum Supression (NMS) is a method to generate \textit{exactly} one detection per object by assuming that highly overlapping detection belong to the same object and collapse them into one detection. NMS accepts the highest scoring detection, then rejects all detection that overlap more than some threshold $\theta$ and repeats the procedure with the remaining detection \cite{hosang_learning_2017}.
...
\textbf{Weighted Box Fusion (WBF)} menggabungkan kotak prediksi dari berbagai model dengan menghitung rata-rata posisi bounding box, diatur berdasarkan skor kepercayaan (\textit{confidence score}) masing-masing prediksi. Pendekatan ini mempertahankan informasi dari seluruh prediksi relevan dan dapat menghasilkan kotak deteksi yang lebih presisi, khususnya ketika model memberikan prediksi yang sedikit berbeda.

\textbf{Non-Maximum Suppression (NMS)} menyeleksi prediksi dengan skor kepercayaan tertinggi dan mengeliminasi kotak yang memiliki tumpang tindih (\textit{overlap}) melebihi \textit{IoU threshold} tertentu. Metode ini efisien secara komputasi namun berpotensi kehilangan prediksi valid yang saling tumpang tindih.

\textbf{Non-Maximum Weighted (NMW)} merupakan varian dari NMS yang mempertimbangkan bobot skor kepercayaan untuk setiap prediksi saat melakukan \textit{suppressi}. Alih-alih menghapus sepenuhnya kotak yang tumpang tindih, NMW menyesuaikan posisi dan ukuran bounding box berdasarkan bobot, sehingga informasi prediksi tambahan tetap dipertahankan.



\subsection{Evaluation metrics}
% TODO: Definisi mAP dan mAR
Kinerja ensemble dibandingkan dengan model tunggal menggunakan metrik \textit{mean Average Precision (mAP)}, \textit{precision}, dan \textit{recall} untuk menilai perbaikan generalisasi dan akurasi deteksi.
